...
class _WeeklyTransactionsStatisticsCardState
      ...
      child: Column(
        children: [
          ...
          LayoutBuilder(builder: (context, constraints) {
            ...
            var selectedBarGroups = pageIndex == 0 ? barGroups.take(7).toList() : barGroups.sublist(pageIndex * 7, ((pageIndex * 7) + 7) < barGroups.length ?  ((pageIndex * 7) + 7) : barGroups.length);
            print("Selected Bar Groups pageIndex ${pageIndex * 7} ${pageIndex * 7 + 7} ${selectedBarGroups.length}");
                ...
                touchCallback: (FlTouchEvent event, barTouchResponse) {
                  if (!event.isInterestedForInteractions ||
                    ...
                    setState(() {
                      ...
                      touchedIndex =
                          barTouchResponse.spot!.touchedBarGroupIndex + (pageIndex * 7);
                      print("touched index ${pageIndex} ${touchedIndex}");
                      DateTime from = weeklySummaries[weeklySummaries.keys.toList()[touchedIndex]]?.dateRange.first??DateTime.now();
                      DateTime to = weeklySummaries[weeklySummaries.keys.toList()[touchedIndex]]?.dateRange.last??DateTime.now();
                      String week = " ${DateFormat("${DateFormat.ABBR_WEEKDAY}, ${DateFormat.ABBR_MONTH} ").format(from)} ${from.day} - ${DateFormat(
            ...
            return Container(
              ...
              child: PageView(
                ...
                children: List.generate((barGroups.length / 7).ceil(), (index) => Stack(
                  children: [
                    Positioned.fill(
              ...
              children: [
                SizedBox(
                  ...
                  child: ChartTransactionIndicator(
                    ...
                    amount:touchedIndex == -1 ? "0": "${weeklySummaries[weeklySummaries.keys.toList()[touchedIndex]]?.income??0}",
                  ),
                ),
                SizedBox(
                  width: constr.maxWidth * 0.33,
                  child: ChartTransactionIndicator(
                    ...
                    amount: touchedIndex == -1 ? "0":  "${weeklySummaries[weeklySummaries.keys.toList()[touchedIndex]]?.tripAmount??0}",
                ...
                SizedBox(
                  ...
                  child: ChartTransactionIndicator(
                    ...
                    amount: touchedIndex == -1 ? "0":"${weeklySummaries[weeklySummaries.keys.toList()[touchedIndex]]?.expenseAmount??0}",
                    text: "Total Expenses",
          ...
          Builder(
            builder: (context) {
              List<Transaction> transactions2 = touchedIndex == -1 ? [] : weeklySummaries[weeklySummaries.keys.toList()[touchedIndex]]?.transactions??[];
  ...
  void generateAvailableWeeks() {
    ...
    List<List<DateTime>> dates = TransactionSummaryCubit.getWeeksForRange(DateTime(selectedYear,1,1), DateTime(selectedYear,12,31));
    ...
    dates.forEach((element) {
      ...
      if (widget.summary.keys.any((e) => element.hasDate(e))){
        weeklySummaries.putIfAbsent({
          ...
          var transactionSummary = widget.summary[widget.summary.keys.firstWhere((e) => element.hasDate(e))]!;
