...
class AuthenticationService {
  ...
  Future<ResponseEntity> initiateGoogleSignin() async {
    ...
    try {
      ...
      if (googleSignInAccount != null) {
        ...
        var response = await authenticationClient.googleLogin(GoogleSigninRequest(accessToken: googleSignInAuthentication.accessToken ?? "", idToken: googleSignInAuthentication.idToken ?? ""));
        if (response.isError){
          return ResponseEntity.Error(response.errorMessage??"An error occurred. Please try again");
        }

        setToken(response.data!
          ..putIfAbsent("idToken", () => googleSignInAuthentication.idToken)
          ..putIfAbsent(
            "accessToken", () => googleSignInAuthentication.accessToken)..putIfAbsent("isGoogle", () => true));
        return response;
      } else {
  ...
  Future<ResponseEntity> refreshToken() async {
    ...
    return response;
      ...
      var response = await authenticationClient.googleLogin(GoogleSigninRequest(accessToken:token["accessToken"]?? "", idToken: token["idToken"] ?? ""));
      if (response.isError){
        return ResponseEntity.Error(response.errorMessage??"An error occurred. Please try again");
      }
      ...
      setToken(response.data!
        ...
        ..putIfAbsent(
            "accessToken", () => token["accessToken"])..putIfAbsent("isGoogle", () => true));
      return response;
    }
  }
